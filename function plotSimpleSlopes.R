...
plotSimSlopes <- function(dat = dat, out=out, xvar, mvars, yvar , xmmod = NULL, mymod = NULL, cmvars = NULL, cyvars = NULL)
{
  options(digits=2)
  require(ggplot2)
  ...
  if (length(xmmod)) {
    ...
    if (is.factor(dat[,xmmod])) {  
      if (length(levels(dat[,xmmod])) > 2) { 
        stop("Moderator x-m path is factor with more than two levels")}
      else {
        xmodLevels <- levels(dat[,xmmod])
  ...
  if (length(mymod)) {
    ...
    if (is.factor(dat[,mymod])) {  
      if (length(levels(dat[,mymod])) > 2) { 
        stop("Moderator m-y path is factor with more than two levels")}
      else {
        ymodLevels <- levels(dat[,mymod])
  ...
  xmin <- min(dat[,xvar], na.rm = TRUE)
  xmax <- max(dat[,xvar], na.rm = TRUE)
  miny <- min(dat[,yvar], na.rm = TRUE)
  maxy <- max(dat[,yvar], na.rm = TRUE)
  ...
  if (length(xmmod)) {
    ...
    if (xdichotomous) { modmin <- 0;
                        modmax <- 1 }
       else {           modmin <- mean(dat[,xmmod]) - sd(dat[,xmmod]);
                        modmax <- mean(dat[,xmmod]) + sd(dat[,xmmod]) }
  ...
  # loop over mediators
    ...
    for (i in 1:length(mvars)) {
     ...
     plotdat1[,xmmod] <- as.factor(plotdat1[,xmmod])
      ...
      if (xdichotomous) {legendLabel <- xmodLevels }
         else {legendLabel <- c("1 SD below mean", "1 SD above mean") }
      
    
      g <- ggplot(plotdat1,aes(x=plotdat1[,xvar],y=plotdat1[,yvar], colour=plotdat1[,xmmod], group = plotdat1[,xmmod])) + geom_point() + geom_line()
      g <- g + labs(x = xvar, y = yvar)  + ylim(min(miny,min(plotdat1[,1])), max(maxy,max(plotdat1[,1]))) 
      g <- g + ggtitle(title) + 
           theme(plot.title = element_text(lineheight=.8, face="bold"))
      g <- g + scale_colour_discrete(name  = xmmod, labels=legendLabel)
      plot(g)
  ...
  if (length(mymod)) {
    ...
    if (ydichotomous) { modmin <- 0;
                        modmax <- 1 }
        else {          modmin <- mean(dat[,mymod]) - sd(dat[,mymod])
                        modmax <- mean(dat[,mymod]) + sd(dat[,mymod]) }
    a <- subset(out, grepl("a1", out$label))[,"est"] 
    b <- subset(out, grepl("b", out$label))[,"est"] 
...
} # end function plotSimSlopes



## test

 plotSimSlopes(dat = dat1, out=out, xvar="x1", mvars= mvars, yvar = "y1", xmmod = "bimod1")
