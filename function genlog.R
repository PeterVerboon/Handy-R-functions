...
# Generalized logistic fit function for single case designs
#
# parameters
...
# input: dataset with two or three columns: (1) measurement numbering, (2) scores and (3) phase indictor called "phase" (0,1)
...
out <- nlsLM(GLF, data=dat, start=list(x0 = 4, B = 0.0, At = max(dat$y), Ab = min(dat$y), v = 1), 
             lower = c((min(dat$x)+2), -2, (max(dat$y)-2), (min(dat$y)-1),1), 
             upper = c((max(dat$x)-10), 2, (max(dat$y)+1), (min(dat$y)+2),1)) 
...
if (plot == TRUE) {
  ...
  g <- g + geom_point()
  g <- g + geom_hline(yintercept=Ab, colour="blue")  
  g <- g + geom_hline(yintercept=At, colour="blue") 
  g <- g + geom_vline(xintercept=x0, colour="green") 
  g <- g + geom_vline(xintercept=nA, colour="grey") 
  g <- g + geom_line(data=data.frame(x=dat$x, y=yfit), aes(x=x,y=y), colour="red", size = .5) 
  g <- g + labs(x = "Measurements", y = "Score")
